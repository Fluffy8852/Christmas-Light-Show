
### My new light show uses Xlights and WLED controls. This project has been on going and updating since 2023. There are 2 annual shows: Haloween and Christmas.

#### 1. Connecting WLED lights

Used light strips for windows, and pixels for bushes. Each of the windows got a 5v power supply which is show in photo bellow which powers both the lights and the controller. (green wire is for data)
<img width="404" height="263" alt="Screenshot 2026-04-28 101540" src="https://github.com/user-attachments/assets/0b5f13df-2d4f-4900-be40-faf660b1b7b5" />



#### 2. Connecting other devices

Built my own outlet box that controls basic lights on and off.
Here is the wiring diagram, power comes in 120v and out 120v, just the one wire goes through the relay. 
<img width="297" height="251" alt="Screenshot 2026-04-28 092226" src="https://github.com/user-attachments/assets/d95f2309-00c8-49e5-a027-8845bde9c228" />


#### 3. Falcon Player (FPP) and FM Radio

Raspiberry Pi 3b+ flashed with FPP connects to each controller IP address. The Pi is connected to an FM radio which allows people to listen in there cars.



#### 4. Xlights

Took photo of house and then mapped out where the lights were refence to the photo. Then connected controllers via IP, and grouped each string of lights together to something that made sense to me for effects later. Now was able to start programing the music to the lights. (This part takes the most ammount of time about hour for each minute of song)



#### 5. Sending Xlights Sequence to FPP

First, start with high resolution render, then connect to FPP to send file.



#### 6. Enjoy

Here are my 2 most recent shows

##### Haloween 2025: 

https://youtu.be/pX\_UAeRBoAQ?si=xViTBxOyZJiqW\_Nv

##### Christmas 2025: 

https://youtu.be/yxuKoiHsfZE?si=1tZTKEKJomIeL5Q1

